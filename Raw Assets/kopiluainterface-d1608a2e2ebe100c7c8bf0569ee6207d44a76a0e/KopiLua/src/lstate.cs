	...
	public partial class Lua
	{
		...
		*/
		public class CallInfo : ArrayElement
		{
			...
			{
				Lua.Assert(ci.values == values);
				return ci.index;
			}

			public static int operator -(CallInfo ci1, CallInfo ci2)
			{
				Lua.Assert(ci1.values == ci2.values);
		...
		*/
        ...
        //  public lua_EasyHook easyHook; /*Added by Diarmid Campbell to give easy entry point into debugger*/
        };
		...
		*/
		...
		{
			Lua.Assert(LUAI_EXTRASPACE == 0, "LUAI_EXTRASPACE not supported");
			return (lua_State)l;
		}
