...
namespace LuaInterface 
{
  ...
  //  using KopiLua;
	...
	public class Lua : IDisposable
	{
        ...
        //This AutoResetEvent starts out non-signalled
        public static AutoResetEvent workerThreadBlocker = new AutoResetEvent(false);
        int asyncNumInstructions;
        Exception asyncException;

        public void DoFileASync(string filename, int nunInstructions)
        {
        ...
        public void SetAsycNumInstructions(int numInstructions)
        {
            asyncNumInstructions = numInstructions;
        ...
        void Run()
        {
            ...
            isFinishedASync = true;
            mainThreadBlocker.Set();
        }

        public void InternalHook(KopiLua.Lua.lua_State L, KopiLua.Lua.lua_Debug ar)
        {
         //   Console.WriteLine("InternalHook");
            lua_sethook(this.InternalHook, KopiLua.Lua.LUA_MASKCOUNT, asyncNumInstructions);

            mainThreadBlocker.Set();
            workerThreadBlocker.WaitOne();
            if (triggerAbortAsync)
            {
               // ThrowExceptionFromError(0);
               throw new LuaException("Aborting Async Lua VM");
            }
        ...
        public int lua_sethook(KopiLua.Lua.lua_Hook func, int mask, int count)
        {
            return LuaDLL.lua_sethook(luaState, func, mask, count);
        }
