...
public class EditorButton : MonoBehaviour {
	...
	void Start () {
		...
		boxLine = new VectorLine("Button box", points, Editor.singleton.pencilLine, Editor.singleton.GetLinePencilLightWidth(), LineType.Continuous, Joins.Weld);
	...
	void Update () {
		...
		boxLine.drawTransform = transform;
		boxLine.lineWidth = lineWidth;
		boxLine.Draw3D();
		pictureGO.GetComponent<EditorModulePicture>().moduleType = moduleType;
		pictureGO.GetComponent<EditorModulePicture>().lineWidth = lineWidth;
		pictureGO.GetComponent<EditorModulePicture>().textColor = textColor;
